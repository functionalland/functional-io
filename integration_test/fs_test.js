...
import { chain, compose } from "https://x.nest.land/ramda@0.27.0/source/index.js";
import { assertIsEquivalent } from "https://deno.land/x/functional@v0.4.2/asserts.js";
...
Deno.test(
  ...
  async () => {
    ...
    await(!(await _exists(`${Deno.cwd()}/dump/hoge`)));
  }
);
...
Deno.test(
  ...
  async () => {
    ...
    const containerA = create(File.fromPath(`${Deno.cwd()}/dump/piyo`))
      .chain(copyFile(File.fromPath(`${Deno.cwd()}/dump/hoge`)))
      .chain(close);
    ...
    const _file = await Deno.open(`${Deno.cwd()}/dump/piyo`);
    const _buffer = await Deno.readAll(_file);
    Deno.close(_file.rid);
...
Deno.test(
  ...
  async () => {
    ...
    const copyToNewFile = compose(
      chain(close),
      chain(copyFile(File.fromPath(`${Deno.cwd()}/dump/hoge`))),
      create
    );

    const containerA = copyToNewFile(File.fromPath(`${Deno.cwd()}/dump/piyo`));
